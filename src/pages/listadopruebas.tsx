...
const Page1: React.FC = () => {
  ...
  const saveItem = async (newItem: { nombre: string; descripcion: string }) => {
    try {
      const response = await axios.post(`${API_URL}/pruebas`, newItem);
      const createdItem = response.data; // Objeto con `id` devuelto por el backend
      setItems([...items, createdItem]); // Agrega el nuevo objeto al estado
      setShowToast(true);
    } catch (error) {
